...
public class MenuServiceImpl extends BaseService<Menu> implements MenuService {
	...
	public int findLastNum(String code) {
		try{
			...
			Menu menu = blade.findFirstBy("pCode = #{pCode} order by num desc", Maps.create().set("pCode", code));
	...
	public boolean isExistCode(String code) {
		...
		boolean temp = blade.isExist(sql, Maps.create().set("code", code));
		return temp;
	}

	@Override
	public boolean updateStatus(String ids, Object status) {
		Maps paras = Maps.create().set("status", status).set("ids", ids.split(","));
