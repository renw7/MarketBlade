...
public class CacheController extends BladeController {
	...
	public AjaxResult getBtn() {
		...
		Map<String, Object> userRole = CacheKit.get(MENU_CACHE, "roleExt_" + userId,
				new ILoader() {
					public Object load() {
						return Db.selectOne("select * from TFW_ROLE_EXT where userId= #{id}", Record.create().set("id", userId));
					}
				});
	...
	public AjaxResult getSelect() {
		...
		List<Map<String, Object>> dict = CacheKit.get(DICT_CACHE, "dict_common_" + code,
				new ILoader() {
					public Object load() {
						return Db.selectList("select num as ID,pId as PID,name as TEXT from  TFW_DICT where code=#{code} and num>0", Record.create().set("code", code));
					}
				});
	...
	public AjaxResult getCombo() {
		...
		List<Map<String, Object>> dict = CacheKit.get(DICT_CACHE, "dict_combo_" + code,
				new ILoader() {
					public Object load() {
						return Db.selectList("select num as \"id\",name as \"text\" from  TFW_DICT where code=#{code} and num>0", Record.create().set("code", code), new AopContext("ztree"));
					}
				});
	...
	public AjaxResult getDeptSelect() {
		...
		List<Map<String, Object>> dept = CacheKit.get(DEPT_CACHE, "dept_all",
				new ILoader() {
					public Object load() {
						return Db.selectList("select ID,PID,simpleName as TEXT from  TFW_DEPT order by pId,num asc", Record.create(), new AopContext(), Cst.me().getDefaultSelectFactory().deptIntercept());
					}
				});
		StringBuilder sb = new StringBuilder();
		sb.append("<select class=\"form-control\" style=\"margin-left:-3px;cursor:pointer;\" id=\"inputs"
	...
	public AjaxResult getUserSelect() {
		...
		List<Map<String, Object>> dept = CacheKit.get(DEPT_CACHE, "user_all",
				new ILoader() {
					public Object load() {
						return Db.selectList("select ID,name TEXT from TFW_USER where status=1 and name is not null order by name ", Record.create(), new AopContext(), Cst.me().getDefaultSelectFactory().userIntercept());
					}
				});
		StringBuilder sb = new StringBuilder();
		sb.append("<select class=\"form-control\" style=\"margin-left:-3px;cursor:pointer;\" id=\"inputs"
	...
	public AjaxResult getRoleSelect() {
		...
		List<Map<String, Object>> dept = CacheKit.get(ROLE_CACHE, "role_all",
				new ILoader() {
					public Object load() {
						return Db.selectList("select ID,name TEXT from TFW_Role where  name is not null order by name ", Record.create(), new AopContext("ztree"));
					}
				});
		StringBuilder sb = new StringBuilder();
		sb.append("<select class=\"form-control\" style=\"margin-left:-3px;cursor:pointer;\" id=\"inputs"
	...
	public AjaxResult dicTreeList() {
		List<Map<String, Object>> dic = CacheKit.get(DICT_CACHE, "dict_all",
				new ILoader() {
					public Object load() {
						return Db.selectList("select code \"code\",id \"id\",pId \"pId\",name \"name\",num \"num\",'false' \"open\" from TFW_DICT order by code asc,num asc", Record.create());
					}
				});
	...
	public AjaxResult deptTreeList() {
		List<Map<String, Object>> dept = CacheKit.get(DEPT_CACHE, "dept_tree_all_" + ShiroKit.getUser().getId(),
				new ILoader() {
					public Object load() {
						return Db.selectList("select id \"id\",pId \"pId\",simpleName as \"name\",(case when (pId=0 or pId is null) then 'true' else 'false' end) \"open\" from  TFW_DEPT ", Record.create(), new AopContext("ztree"), Cst.me().getDefaultSelectFactory().deptIntercept());
					}
				});
	...
	public AjaxResult roleTreeList() {
		List<Map<String, Object>> dept = CacheKit.get(ROLE_CACHE, "role_tree_all_" + ShiroKit.getUser().getId(),
				new ILoader() {
					public Object load() {
						return Db.selectList("select id \"id\",pId \"pId\",name as \"name\",(case when (pId=0 or pId is null) then 'true' else 'false' end) \"open\" from  TFW_ROLE ", Record.create(), new AopContext("ztree"), Cst.me().getDefaultSelectFactory().roleIntercept());
					}
				});
	...
	public AjaxResult getDicById() {
		...
		List<Map<String, Object>> dict = CacheKit.get(DICT_CACHE, "dict_" + id,
				new ILoader() {
					public Object load() {
						return Db.selectList("select CODE from TFW_DICT where id=#{id}",Record.create().set("id", id), new AopContext("ztree"));
					}
				});
		return json(dict);
	}
