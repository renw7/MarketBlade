...
public class ShiroUser implements Serializable {
	...
	public ShiroUser(Object id, Object deptId, String loginName, String name, List<String> roleList) {
		...
		if (Func.isOracle()) {
			...
			subDepts = Db.init().queryStr(deptSql, Maps.create().set("deptIds", deptId.toString().split(",")));
		} else {
			String[] arr = deptId.toString().split(",");
			StringBuilder sb = new StringBuilder();
			for (String deptid : arr) {
				deptSql = "select queryChildren(#{deptid},'tfw_dept') as subdepts";
				String str = Db.init().queryStr(deptSql, Maps.create().set("deptid", deptid));
		...
		if (Func.isOracle()) {
			...
			subRoles = Db.init().queryStr(roleSql, Maps.create().set("roleIds", roleList));
		} else {
			StringBuilder sb = new StringBuilder();
			for (String roleid : roleList) {
				roleSql = "SELECT queryChildren(#{deptid},'tfw_role') as subroles";
				String str = Db.init().queryStr(roleSql, Maps.create().set("deptid", roleid));
				sb.append(str).append(",");
			}
